...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = await request.json() as { phoneNumberId?: string; doctorId?: string; date?: string };
    const phoneNumberId = body.phoneNumberId?.trim() ?? ""; const date = body.date?.trim() ?? "";
    if (!/^\d+$/.test(phoneNumberId) || !body.doctorId || !validDate(date)) return NextResponse.json({ error: "phoneNumberId, doctorId, and a valid future date (YYYY-MM-DD) are required." }, { status: 400 });
