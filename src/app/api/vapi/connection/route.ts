...
async function configureVapi(apiKey: string, assistantId: string, serverUrl: string, greeting: string) {
  ...
  const update = await fetch(`https://api.vapi.ai/assistant/${encodeURIComponent(assistantId)}`, { method: "PATCH", headers, body: JSON.stringify({ model, firstMessage: greeting, firstMessageMode: "assistant-speaks-first", keypadInputPlan: { enabled: true, delimiters: [""], timeoutSeconds: 2 }, server: { url: serverUrl, timeoutSeconds: 20 }, serverMessages: ["tool-calls", "function-call", "end-of-call-report", "status-update"] }) });
