...
const defaultSettings: HospitalSettings = { hospital_name: "ABC Hospital", hospital_logo: null, departments: [], opening_time: "09:00", closing_time: "17:00", slot_duration: 20, emergency_number: null, whatsapp_number: null, receptionist_number: null, chat_retention_hours: 24 };
...
export default function DashboardClient({ hospitalId, initialConversations, initialMessages, initialAppointments, initialDoctors, initialPatients, initialSettings }: { hospitalId: string; initialConversations: Conversation[]; initialMessages: Message[]; initialAppointments: Appointment[]; initialDoctors: Doctor[]; initialPatients: Patient[]; initialSettings: HospitalSettings | null }) {
  ...
  async function deleteDoctor(doctor: Doctor) { if (!confirm(`Delete ${doctor.name}? Existing appointment history will be retained, but this doctor will no longer be available for new bookings.`)) return; const response = await fetch(`/api/doctors/${doctor.id}`, { method: "DELETE" }); const body = await response.json() as { error?: string }; if (!response.ok) { alert(body.error ?? "Unable to delete doctor."); return; } setDoctors((items) => items.filter((item) => item.id !== doctor.id)); router.refresh(); }
  ...
  else content = <><div className="page-title"><div><div className="eyebrow">ADMINISTRATION</div><h1>Hospital Settings</h1></div></div><section className="panel settings"><label>Hospital name<input value={settings.hospital_name} onChange={(event) => setSettings({ ...settings, hospital_name: event.target.value })} /></label><label>Departments (comma separated)<input value={settings.departments.join(", ")} onChange={(event) => setSettings({ ...settings, departments: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) })} /></label><label>Receptionist phone number<input type="tel" value={settings.receptionist_number ?? ""} placeholder="+91 98765 43210" onChange={(event) => setSettings({ ...settings, receptionist_number: event.target.value || null })} /></label><label>Opening time<input type="time" value={settings.opening_time.slice(0, 5)} onChange={(event) => setSettings({ ...settings, opening_time: event.target.value })} /></label><label>Closing time<input type="time" value={settings.closing_time.slice(0, 5)} onChange={(event) => setSettings({ ...settings, closing_time: event.target.value })} /></label><label>Slot duration (minutes)<input type="number" value={settings.slot_duration} onChange={(event) => setSettings({ ...settings, slot_duration: Number(event.target.value) })} /></label><label>Chat and contact retention (hours)<input type="number" min="1" max="720" value={settings.chat_retention_hours ?? 24} onChange={(event) => setSettings({ ...settings, chat_retention_hours: Math.min(720, Math.max(1, Number(event.target.value) || 24)) })} /><small>Default: 24 hours. Chats are cleared automatically; patient records with appointments are retained for the dashboard.</small></label><button className="primary small" onClick={saveSettings}>Save settings</button></section></>;
