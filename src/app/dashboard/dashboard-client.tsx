...
const navigation: View[] = ["Dashboard", "Appointments", "Doctors", "Patients", "WhatsApp Chats", "Calendar", "WhatsApp Connection", "Voice Agent", "Hospital Settings"];
...
export default function DashboardClient({ hospitalId, initialConversations, initialMessages, initialAppointments, initialDoctors, initialPatients, initialSettings }: { hospitalId: string; initialConversations: Conversation[]; initialMessages: Message[]; initialAppointments: Appointment[]; initialDoctors: Doctor[]; initialPatients: Patient[]; initialSettings: HospitalSettings | null }) {
  ...
  return <main className={`hospital ${styles.responsive}`} data-theme={dark ? "dark" : "light"}><aside className="main-nav"><div className="hospital-brand"><span>✚</span><b>{settings?.hospital_name ?? "ABC Hospital"}</b><small>Appointment assistant</small></div>{navigation.map((item) => <button key={item} onClick={() => setView(item)} className={view === item ? "active" : ""}>{item}</button>)}<button onClick={() => setDark((value) => !value)}>{dark ? "☀ Light mode" : "◐ Dark mode"}</button><button className="signout" onClick={signOut}>Sign out</button></aside><section className="content">{content}</section>{viewingAppointment && <div className={styles.modalBackdrop} role="dialog" aria-modal="true"><section className={styles.modal}><button className={styles.close} onClick={() => setViewingAppointment(null)} aria-label="Close patient details">×</button><div className={styles.modalEyebrow}>PATIENT APPOINTMENT</div><h2>{viewingAppointment.patient_name}</h2><p className={styles.modalStatus}>{viewingAppointment.status === "completed" ? "Checkup complete" : viewingAppointment.status === "cancelled" ? "Cancelled" : "Checkup incomplete"}</p><dl><div><dt>Phone</dt><dd>{viewingAppointment.phone_number}</dd></div><div><dt>Doctor</dt><dd>{viewingAppointment.doctor_name ?? "Not assigned"}</dd></div><div><dt>Department</dt><dd>{viewingAppointment.department ?? "Not assigned"}</dd></div><div><dt>Date</dt><dd>{viewingAppointment.appointment_date}</dd></div><div><dt>Time</dt><dd>{time(viewingAppointment.appointment_time)}</dd></div></dl><button className="primary small" onClick={() => setViewingAppointment(null)}>Close</button></section></div>}</main>;
}
